defmodule Exenvoice.InvoicesTest do
  ...
  describe "invoices" do
    ...
    test "create_invoice/1 with valid data creates a invoice" do
      valid_attrs = %{no: "some no", date: ~D[2025-09-26], year_month: ~D[2025-09-26], diagnosis: "some diagnosis", sum_events: true}

      assert {:ok, %Invoice{} = invoice} = Invoices.create_invoice(valid_attrs)
      assert invoice.no == "some no"
      assert invoice.date == ~D[2025-09-26]
      assert invoice.year_month == ~D[2025-09-26]
      assert invoice.diagnosis == "some diagnosis"
      assert invoice.sum_events == true
    end
    ...
    test "update_invoice/2 with valid data updates the invoice" do
      ...
      assert invoice.no == "some updated no"
      assert invoice.date == ~D[2025-09-27]
      assert invoice.year_month == ~D[2025-09-27]
      assert invoice.diagnosis == "some updated diagnosis"
      assert invoice.sum_events == false
    end
